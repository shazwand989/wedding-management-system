            ...
            <div class="modal-content">
                ...
                <form id="paymentForm">
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="amount" class="control-label">Amount (RM) *</label>
                            <input type="number" class="form-control" id="amount" name="amount" min="0.01" step="0.01" required>
                            <span class="help-block" id="balanceInfo"></span>
                        </div>
    ...
    <script>
        $(document).ready(function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                                ...
                                data: `action=update_payment_status&payment_id=${paymentId}&status=${status}`,
                                dataType: 'json'
                            })
                            .done(function(data) {
                                if (data.success) {
            ...
            window.viewPayment = function(paymentId) {
                $.ajax({
                        url: `../includes/ajax_handler.php?action=get_payment_details&id=${paymentId}`,
                        method: 'GET',
                        dataType: 'json'
                    })
                    .done(function(data) {
                        if (data.success) {
                            ...
                            const paymentDetailsModal = new bootstrap.Modal($('#paymentDetailsModal')[0]);
            ...
            $('#booking_id').on('change', function() {
                ...
                if ($option.val()) {
                    const total = parseFloat($option.data('total'));
                    const paid = parseFloat($option.data('paid'));
                    const balance = total - paid;

                    $('#balanceInfo').text(`Remaining balance: RM ${balance.toFixed(2)}`);
                    $('#amount').attr('max', balance).val(balance);
                } else {
                    $('#balanceInfo').text('');
                    $('#amount').removeAttr('max').val('');
                }
            });
            ...
            $('#paymentForm').on('submit', function(e) {
                ...
                const formData = $(this).serialize() + '&action=add_payment';
                ...
                $.ajax({
                    ...
                    .done(function(data) {
                        if (data.success) {
                            const paymentModal = bootstrap.Modal.getInstance($('#paymentModal')[0]);
                            if (paymentModal) {
                                paymentModal.hide();
                            }
                            location.reload();
